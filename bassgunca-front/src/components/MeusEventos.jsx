...
function MeusEventos({ eventos, usuarioLogado, setEventos }) {
  ...
  const temPermissaoProdutor =
    usuarioLogado?.funcoes?.toUpperCase().includes("PRODUTOR") ||
    usuarioLogado?.funcoes?.toUpperCase().includes("EVENTO");

  // 3. Função de salvar (UPDATE - HU 2.3)
  const handleSalvar = async (e) => {
    e.preventDefault();

    // 1. O TRUQUE DA DATA: Garante que a data mestre nunca vá vazia
    const dataPrincipal =
      editando.tipo_evento === "festival" && editando.programacao?.length > 0
    ...
    try {
      const response = await fetch(
        `http://localhost:3000/api/eventos/${editando.id}`,
        {
          method: "PUT",
  ...
  // 4. Nova Função: Apagar Evento (DELETE - HU 2.4)
  const apagarEvento = async (id) => {
    const confirmar = window.confirm(
      "Tem a certeza que deseja apagar este evento do sistema? Esta ação não pode ser desfeita.",
    );
    ...
    if (confirmar) {
      try {
        ...
        if (resposta.ok) {
          ...
          window.location.reload(); // Recarrega a página para remover da lista instantaneamente
        } else {
  ...
  if (editando) {
    ...
    const adicionarDia = () => {
      const diasAtuais = Array.isArray(editando.programacao)
        ? [...editando.programacao]
        : [];
    ...
    return (
      ...
      >
        ...
        >
          ...
          >
            ...
            <label className="fonte-texto" style={{ color: "#666" }}>
              {editando.tipo_evento === "festival"
                ? "Valor do Passaporte (Todos os dias)"
                : "Valor do Ingresso (R$)"}
              <input
                type="number"
                step="0.01"
                ...
                placeholder={
                  editando.tipo_evento === "festival"
                    ? "Ex: 150.00 (Deixe vazio se não houver passaporte)"
                    : "Ex: 40.00"
                }
                style={inputStyle}
              />
            ...
            >
              <label className="fonte-texto" style={{ color: "#666" }}>
                ...
                <input
                  ...
                  onChange={(e) =>
                    setEditando({ ...editando, data_hora: e.target.value })
                  }
                  style={inputStyle}
                  required
                />
          ...
          >
            <label className="fonte-texto" style={{ color: "#888" }}>
              Nome do Evento *
              <input
                type="text"
            ...
            <label className="fonte-texto" style={{ color: "#888" }}>
              ...
              <input
                ...
                onChange={(e) =>
                  setEditando({ ...editando, imagem_url: e.target.value })
                }
                style={inputStyle}
              />
            </label>
          ...
          >
            ...
            <label className="fonte-texto" style={{ color: "#888" }}>
              Localização (Google Maps URL)
              <input
                type="url"
                value={editando.localizacao_url || ""}
                onChange={(e) =>
                  setEditando({ ...editando, localizacao_url: e.target.value })
                }
                style={inputStyle}
              />
            </label>
            <label className="fonte-texto" style={{ color: "#888" }}>
              Link do Ingresso / Shotgun
              <input
                type="url"
                value={editando.link_ingresso || ""}
                onChange={(e) =>
                  setEditando({ ...editando, link_ingresso: e.target.value })
                }
                style={inputStyle}
              />
            </label>
            ...
            >
              ...
              >
                <div>
                  <h3
                    className="fonte-quadrada"
                    style={{ color: "#fff", margin: "0 0 5px 0" }}
                  >
                    BILHETES DIÁRIOS E PROGRAMAÇÃO
                  </h3>
                  <p
                    className="fonte-texto"
                    style={{ color: "#888", fontSize: "0.85rem", margin: 0 }}
                  >
                    Adicione os dias específicos do festival para quem quer
                    comprar avulso.
                  </p>
                </div>
                <button
              ...
              {Array.isArray(editando.programacao) &&
                editando.programacao.map((dia, index) => (
                  <div
                    key={index}
                    style={{
                      display: "grid",
                      gridTemplateColumns:
                        "minmax(150px, 1fr) minmax(100px, 1fr) minmax(200px, 2fr) auto",
                      gap: "15px",
                      marginBottom: "15px",
                      alignItems: "end",
                      background: "#050505",
              ...
              {(!editando.programacao || editando.programacao.length === 0) && (
                ...
                >
                  ...
                  >
                    Nenhum dia de festival configurado.
                  </p>
                </div>
              )}
            </div>
          ...
          <div style={{ display: "grid", gap: "20px" }}>
            <label className="fonte-texto" style={{ color: "#888" }}>
              ...
              <input
                type="text"
                value={editando.generos || ""}
                onChange={(e) =>
                  setEditando({ ...editando, generos: e.target.value })
                }
                placeholder="Ex: Hard Techno, UK Garage, Dubstep..."
                style={inputStyle}
              />
            </label>

            <label className="fonte-texto" style={{ color: "#888" }}>
              Line-up Geral (Vulgos dos Artistas separados por vírgula) *
              <input
                type="text"
                value={editando.lista_artistas || ""}
                onChange={(e) =>
                  setEditando({ ...editando, lista_artistas: e.target.value })
                }
                placeholder="Ex: Bea, Dj Caos, W3LLY"
                style={inputStyle}
                required
              />
            </label>
            ...
            <label className="fonte-texto" style={{ color: "#888" }}>
              ...
              <textarea
                ...
                onChange={(e) =>
                  setEditando({ ...editando, informacoes: e.target.value })
                }
  ...
  if (meusEventos.length === 0) {
    return (
      <div style={{ textAlign: "center", marginTop: "100px" }}>
        <h2 className="fonte-quadrada" style={{ color: "#fff" }}>
        ...
        >
          Clique no botão "Novo Evento" no topo para começar.
        </p>
      </div>
    );
  }

  // --- RENDER 4: LISTA DE EVENTOS COM BOTÃO DE EDIÇÃO E APAGAR ---
  return (
    <div style={{ padding: "40px" }}>
      <h2
        className="fonte-quadrada"
        style={{ color: "#fff", fontSize: "2.5rem", marginBottom: "30px" }}
      >
        MEUS EVENTOS
      </h2>
      <div
        ...
        {meusEventos.map((e) => (
          ...
          >
            ...
            >
              <span
                style={{
                  background: e.tipo_evento === "festival" ? "#ff003c" : "#111",
                  color: "#fff",
                  padding: "2px 8px",
                  fontSize: "0.7rem",
                  borderRadius: "4px",
            ...
            <p
              className="fonte-texto"
              style={{ color: "#666", margin: "0 0 5px 0" }}
            >
              📅 {new Date(e.data_hora).toLocaleDateString("pt-BR")}
            </p>
            <p
              className="fonte-texto"
              style={{ color: "#666", margin: "0 0 15px 0" }}
            >
              ...
              <button
                onClick={() => {
                  ...
                  const formataSeguro = (dataOriginal) => {
                    ...
                    if (
                      typeof dataOriginal === "string" &&
                      dataOriginal.includes("T") &&
                      dataOriginal.length === 16
                    )
                      return dataOriginal;
                    try {
                  ...
                  try {
                    programacaoArray =
                      typeof e.programacao === "string"
                        ? JSON.parse(e.programacao)
                        : e.programacao || [];
...
const TelaMeusEventosVazia = () => {
  return (
    ...
    >
      ...
      <h1
        className="fonte-quadrada"
        style={{ color: "#fff", fontSize: "2.5rem", marginBottom: "10px" }}
      >
        PAINEL DO PRODUTOR
      </h1>
      ...
      >
        Parece que você ainda não tem permissão para gerenciar eventos. Esta
        área é exclusiva para quem movimenta a cena e organiza os corre.
      </p>
