...
const abaStyle = { background: 'transparent', border: 'none', padding: '15px 5px', fontSize: '0.9rem', cursor: 'pointer', transition: '0.2s', letterSpacing: '1px' };
...
function MeuPerfil({ usuarioLogado, setUsuarioLogado, eventos }) {
  ...
  useEffect(() => {
    const carregarDadosFrescos = async () => {
      try {
        const resposta = await fetch(`http://localhost:3000/api/usuarios/buscar/${encodeURIComponent(usuarioLogado?.vulgo || '')}`);
        if (resposta.ok) {
  ...
  // FUNÇÕES / TAGS
  // =========================
  const funcoesStr = dadosCompletos?.funcoes || dadosCompletos?.funcao || 'MEMBRO';
  const listaTags = funcoesStr.split(',').filter(f => f.trim() !== '');

  const funcoesUpper = funcoesStr.toUpperCase();
  const ehProdutor = funcoesUpper.includes('PRODUTOR') || funcoesUpper.includes('EVENTO');

  // =========================
  // EVENTOS
  // =========================
  const meuVulgo = String(dadosCompletos?.vulgo || '').trim().toLowerCase();

  const eventosProdutor = (eventos || []).filter(e =>
    String(e.criado_por || '').trim().toLowerCase() === meuVulgo
  );

  const eventosLineup = (eventos || []).filter(e => {
    if (!meuVulgo) return false;
    const regex = new RegExp(`\\b${meuVulgo}\\b`, 'i');
    return regex.test(String(e.titulo || '')) ||
           regex.test(String(e.lista_artistas || e.programacao || ''));
  });

  const eventosInteressado = (eventos || []).filter(e =>
    meuVulgo && String(e.interessados || '').toLowerCase().includes(meuVulgo)
  );
    ...
    <div style={{ padding: '40px 20px', maxWidth: '1000px', margin: '0 auto', color: '#fff', paddingBottom: '100px' }}>
      ...
      <div style={{ background: '#050505', padding: '40px', borderRadius: '16px', border: '1px solid #1a1a1a', display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', flexWrap: 'wrap', gap: '20px' }}>
        ...
        <div style={{ display: 'flex', gap: '30px', alignItems: 'center', flexWrap: 'wrap' }}>
          <img
            src={dadosCompletos?.foto_perfil || 'https://via.placeholder.com/150'}
            alt="Perfil"
          ...
          <div>
            ...
            <div style={{ display: 'flex', gap: '8px', flexWrap: 'wrap', marginBottom: '15px' }}>
              {listaTags.map((f, i) => (
                <span key={i} className="fonte-quadrada" style={{ background: '#1a1a1a', padding: '4px 10px', borderRadius: '4px', fontSize: '0.7rem', color: '#ff003c', border: '1px solid #ff003c' }}>
                  {f.trim().toUpperCase()}
                </span>
              ))}
            </div>
            ...
            <div style={{ display: 'flex', gap: '20px' }}>
              {links.instagram && <a href={links.instagram} target="_blank" rel="noreferrer" style={{ color: '#888', transition: '0.3s' }} onMouseEnter={e => e.currentTarget.style.color = '#ff003c'} onMouseLeave={e => e.currentTarget.style.color = '#888'}><FaInstagram size={26} /></a>}
              {links.soundcloud && <a href={links.soundcloud} target="_blank" rel="noreferrer" style={{ color: '#888', transition: '0.3s' }} onMouseEnter={e => e.currentTarget.style.color = '#ff003c'} onMouseLeave={e => e.currentTarget.style.color = '#888'}><FaSoundcloud size={26} /></a>}
              {links.spotify && <a href={links.spotify} target="_blank" rel="noreferrer" style={{ color: '#888', transition: '0.3s' }} onMouseEnter={e => e.currentTarget.style.color = '#1DB954'} onMouseLeave={e => e.currentTarget.style.color = '#888'}><FaSpotify size={26} /></a>}
              {links.geral && <a href={links.geral} target="_blank" rel="noreferrer" style={{ color: '#888', transition: '0.3s' }} onMouseEnter={e => e.currentTarget.style.color = '#ff003c'} onMouseLeave={e => e.currentTarget.style.color = '#888'}><SiLinktree size={24} /></a>}
            </div>
          </div>
        </div>
        ...
        <button
          ...
          style={{ background: 'transparent', color: '#ff003c', border: '1px solid #ff003c', padding: '10px 20px', borderRadius: '4px', cursor: 'pointer', fontSize: '0.8rem' }}
        >
          EDITAR PERFIL
        </button>
      </div>
          ...
          {eventosExibidos.length === 0 ? (
            ...
            eventosExibidos.map(e => (
              <div key={e.id} style={{ background: '#0a0a0a', padding: '20px', borderRadius: '12px', border: '1px solid #1a1a1a' }}>
                <h4 className="fonte-quadrada" style={{ color: '#ff003c' }}>{e.titulo}</h4>
