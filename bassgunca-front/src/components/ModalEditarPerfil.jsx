  ...
  const [editando, setEditando] = useState(() => {
    ...
    if (rawRedes) {
      try {
        ...
        console.warn("Erro ao processar redes sociais, usando como link geral:", e);
        links.geral = String(rawRedes);
      }
    }
  ...
  const handleSalvar = async (e) => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        const sessionAntiga = JSON.parse(localStorage.getItem('@bassgunca:user_session'));
        if (sessionAntiga) {
          sessionAntiga.usuario = dadosAtualizados;
          localStorage.setItem('@bassgunca:user_session', JSON.stringify(sessionAntiga));
        }
    ...
    <div style={{ padding: '40px 20px', maxWidth: '900px', margin: '0 auto', color: '#fff', paddingBottom: '100px' }}>
      ...
      <form onSubmit={handleSalvar} style={{ display: 'flex', flexDirection: 'column', gap: '30px' }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '150px 1fr', gap: '30px', background: '#0a0a0a', padding: '30px', borderRadius: '12px', border: '1px solid #1a1a1a', alignItems: 'center' }}>
          <img src={editando.foto_perfil || 'https://via.placeholder.com/150'} alt="Preview" style={{ width: '150px', height: '150px', borderRadius: '8px', objectFit: 'cover', border: '2px solid #ff003c' }} />
          <div style={{ display: 'flex', flexDirection: 'column', gap: '15px' }}>
             <label className="fonte-texto" style={labelStyle}>URL DA FOTO DE PERFIL
                <input type="text" value={editando.foto_perfil || ''} onChange={e => setEditando({...editando, foto_perfil: e.target.value})} style={inputStyle} />
             </label>
   ...
   />
           ...
           <div style={{ background: '#0a0a0a', padding: '20px', borderRadius: '12px', border: '1px solid #1a1a1a' }}>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '10px' }}>
                {['Artista', 'Produtor', 'Grupo/Banda', 'Público', 'Evento', 'Outro'].map(f => {
                  const funcoesAtuais = editando.funcoes ? editando.funcoes.split(',') : [];
        ...
        <div style={{ background: '#0a0a0a', padding: '30px', borderRadius: '12px', border: '1px solid #1a1a1a' }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '20px' }}>
            <label className="fonte-texto" style={labelStyle}>INSTAGRAM
              <input type="url" value={editando.link_instagram || ''} onChange={e => setEditando({...editando, link_instagram: e.target.value})} style={inputStyle} />
            </label>
            <label className="fonte-texto" style={labelStyle}>SOUNDCLOUD
              <input type="url" value={editando.link_soundcloud || ''} onChange={e => setEditando({...editando, link_soundcloud: e.target.value})} style={inputStyle} />
            </label>
            <label className="fonte-texto" style={labelStyle}>SPOTIFY
              <input type="url" value={editando.link_spotify || ''} onChange={e => setEditando({...editando, link_spotify: e.target.value})} style={inputStyle} />
            </label>
            <label className="fonte-texto" style={labelStyle}>LINK GERAL
              <input type="url" value={editando.link_geral || ''} onChange={e => setEditando({...editando, link_geral: e.target.value})} style={inputStyle} />
            </label>
          </div>
        </div>
