...
function Artistas({ eventos, abrirPerfilUsuario }) {
  ...
  const rankingArtistas = useMemo(() => {
    ...
    eventos.forEach(evento => {
      ...
      const nomes = evento.lista_artistas.split(',').map(nome => nome.trim().toUpperCase());
      
      // Calcula o hype do evento (quantas pessoas marcaram interesse)
      const hypeDoEvento = evento.interessados && evento.interessados.trim() !== '' 
    ...
    return Object.values(mapaArtistas)
      .sort((a, b) => b.hypeTotal - a.hypeTotal || b.totalEventos - a.totalEventos);
  }, [eventos]);
  ...
  return (
    <div style={{ padding: '40px', maxWidth: '1200px', margin: '0 auto', width: '100%' }}>
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '40px', flexWrap: 'wrap', gap: '20px' }}>
        <h1 className="fonte-quadrada" style={{ fontSize: '3rem', color: '#fff', margin: 0, textTransform: 'uppercase' }}>
          ARTISTAS POPULARES
        </h1>
        
        {/* Busca rápida */}
        <div style={{ position: 'relative', width: '300px' }}>
          <span style={{ position: 'absolute', left: '15px', top: '10px', fontSize: '1rem' }}>🔎</span>
          <input 
            type="text" 
            ...
            style={{
              width: '100%', padding: '12px 15px 12px 40px', fontSize: '1rem',
      ...
      <div style={{ 
        ...
        {artistasFiltrados.map((artista, index) => {
          ...
          let corMedalha = '';
          if (index === 0) corMedalha = '#ffcc00'; // Ouro
          else if (index === 1) corMedalha = '#c0c0c0'; // Prata
          ...
          return (
            ...
            >
              ...
              <div style={{ position: 'relative' }}>
                ...
                {index < 3 && !busca && (
                  <div style={{
                    ...
                    background: corMedalha, color: '#000', width: '24px', height: '24px',
                    borderRadius: '50%', display: 'flex', justifyContent: 'center', alignItems: 'center',
                    fontSize: '0.8rem', fontWeight: 'bold', border: '2px solid #1a1a1a'
                  }}>
                    {index + 1}
                  </div>
                )}
              ...
              {/* Infos do Artista */}
              <div style={{ flex: 1 }}>
                <h3 className="fonte-quadrada" style={{ fontSize: '1.6rem', color: '#fff', margin: '0 0 5px 0' }}>
                  {artista.nome}
                </h3>
                <p className="fonte-texto" style={{ fontSize: '0.9rem', color: '#aaa', margin: 0 }}>
                  <span style={{ color: '#ff003c' }}>{artista.hypeTotal} hype</span> • {artista.totalEventos} {artista.totalEventos === 1 ? 'evento' : 'eventos'} no radar
                </p>
              </div>
            </div>
          );
        ...
        {artistasFiltrados.length === 0 && (
          <p className="fonte-texto" style={{ color: '#666', gridColumn: '1 / -1', textAlign: 'center', padding: '40px' }}>
