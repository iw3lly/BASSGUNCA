...
const usuarioController = {
    ...
    buscarPorVulgo: async (req, res) => {
        ...
        try {
            ...
            const query = 'SELECT id, nome, vulgo, bio, foto_perfil, funcoes, redes_sociais FROM usuarios WHERE vulgo = ?';
            const [usuarios] = await pool.query(query, [vulgo]); 
            
            if (usuarios.length === 0) {
                return res.status(404).json({ erro: "Usuário não encontrado na cena." });
            }

            const usuario = usuarios[0];

            // Se as redes sociais estiverem salvas como texto JSON, converte de volta
            if (usuario.redes_sociais && typeof usuario.redes_sociais === 'string' && usuario.redes_sociais.startsWith('{')) {
                try {
