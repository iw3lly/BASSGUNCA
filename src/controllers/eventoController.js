  ...
  toggleInteresse: async (req, res) => {
    ...
    try {
      const [eventos] = await pool.query(
        "SELECT interessados FROM eventos WHERE id = ?",
        [id],
      );
      if (eventos.length === 0)
        return res.status(404).json({ erro: "Evento não encontrado" });

      let lista = eventos[0].interessados
        ? eventos[0].interessados.split(",")
        : [];
      ...
      await pool.query("UPDATE eventos SET interessados = ? WHERE id = ?", [
        novaString,
        id,
      ]);
